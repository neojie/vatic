...
#        self.sliceTra()
    ...
    def readxdat(self):
        ...
        for i in np.arange(np.size(self.eleName)):
            ele_index = [self.eleName[i]]*self.eleNum[i]+ele_index
    ...
    def sliceTra(self,ax,ele,colorlist,atom,i):
        """ 
    ...
    #                    print("atom_iloc",atom_iloc)
                ax.plot(atom_iloc['x'], atom_iloc['y'], atom_iloc['z'],color=colorlist[i],label=ele)
                handles, labels = plt.gca().get_legend_handles_labels()
                by_label = OrderedDict(zip(labels, handles))
                plt.legend(by_label.values(), by_label.keys())
            
    def viewTra(self,ele='All',atom_index='All',color=None):
        if color==None:
        ...
        if ele is 'All' and (atom_index=='All'):
            ele = self.eleName       
            for i in np.arange(np.size(ele)):
                if i == 0:
                ...
                for j in atom_list:
                    ...
                    self.sliceTra(ax,ele[i],colorlist,atom,i)
                    
        elif (ele !='All') and (atom_index=='All'):
            for i in np.arange(np.size(self.eleName)):
                if self.eleName[i] == ele:
                    ele_index = i;
                    if i == 0:
                        atom_list = np.arange(self.eleNum[i])+1
                    else:
            ...
            for j in atom_list:
                atom = self.car.loc(axis=0)[:, :, j]
                self.sliceTra(ax,ele,colorlist,atom,ele_index)
            ...
            print(ele)
            self.sliceTra(ax,ele,colorlist,atom,0)
        else:
            print("input wrong")
...
if __name__ == '__main__':
    inp = trajxdt(stepsize=3000)
    inp.viewTra(atom_index=80)
