...
class XDATCAR():
    def __init__(self,file = 'XDATCAR'):
        ...
        # modified by Jie change XDATCAR to file
        if os.path.exists(self.file):
            self.XDATCAR=open(self.file,'r')
        else:
        ...
        print("main loop, uprange is",self.uprange)
    ...
    def step_select(self,selected_step): # 't>100ps and t < 1000ps'
        ...
        print("in step_select, upper is", self.uprange)
        print("selected_step is", selected_step)
    ...
    def writevasp(self,pdb_frame,target='POSCAR.pdb'):
        """
        added by Jie, to build single frame vasp file
        """
        tobewriten=[]
        tobewriten.append(self.title)
        tobewriten.append("   "+str(self.scaling_factor))
        tobewriten.append('%4s%8.6f%4s%8.6f%4s%8.6f'%('    ',self.lattice[0][0],'    ',self.lattice[0][1],'    ',self.lattice[0][2]))
        tobewriten.append('%4s%8.6f%4s%8.6f%4s%8.6f'%('    ',self.lattice[1][0],'    ',self.lattice[1][1],'    ',self.lattice[1][2]))
        tobewriten.append('%4s%8.6f%4s%8.6f%4s%8.6f'%('    ',self.lattice[2][0],'    ',self.lattice[2][1],'    ',self.lattice[2][2]))
        tmp_ele = ''
        tmp_num = ''
        for i in range(len(self.element_list)):
            tmp_ele += '    '+self.element_list[i]
            tmp_num += '    '+str(self.element_amount[i])
        tobewriten.append(tmp_ele)
        tobewriten.append(tmp_num)        
        tobewriten.append('Cart')
        #print(len(self.total_elements))
        for i in range(len(self.total_elements)):
            tobewriten.append('%4s%8.8f%4s%8.8f%4s%8.8f' \
                %('    ',self.cartesian_position[i][0],'    ',\
                self.cartesian_position[i][1],'    ',self.cartesian_position[i][2]))
        with open(target,'a+') as pdbwriter:
            tobewriten=[i+'\n' for i in tobewriten]
            pdbwriter.writelines(tobewriten)

    def writepdb(self,pdb_frame,target='POSCAR.pdb'):
        tobewriten=[]
