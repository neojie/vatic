...
#  
    ...
    def cal_I(self):
        """
        calculate the thermal radiance
        trap : calculate the area
        """
        N        = len(self.lamda)
        self.I   = np.ones(N)
        for i in range(N):  
            temp = planck(self.lamda[i],self.T) 
            zI   = self.zA[:,i]*temp*np.exp(-self.ztau[:,i]);
            self.I[i] = np.trapz(zI,self.z)
        plt.figure()
        plt.title("kEexp()")
        plt.plot(self.z*1e6,zI)
    ...
    def cal_I2(self,side='positive'):
        ...
        self.I2   = np.ones(N)
        for i in range(N):
            self.I2[i] = odeint(self.model,I0,z,args=(self.lamda[i],side))[-1]
        plt.figure()
        plt.plot(self.lamda,self.I2)
